...
export async function withdrawAll(orderId: number) {
    ...
    if (clientPk && tokenAddr && walletsPK && walletsSK) {
        ...
        for (i = 0; i < walletsPK.length; i++) {
            console.log(`Starting withdrawal for bot #${i} from withdraw all func()...`)
            const isWithdrawn = await withdrawBot(walletsSK[i], walletsPK[i], clientPk, tokenAddr, i);
            if (!isWithdrawn) {
                console.error(`Failed to withdraw balance from bot #${i} wallet: ${walletsPK[i]}`)
            }
